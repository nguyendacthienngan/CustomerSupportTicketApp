...
class Dashboard(DashboardTemplate):
  ...
  def __init__(self, **properties):
    ...
    self.set_event_handler('x-open-tickets', self.open_tickets)
  ...
  def change_date_range(self, sender, **event_args):
    if sender.date is None:
      alert("Please select a start and end date for your Dashboard data")
    else:
      self.initialise_dashboard_data()
  ...
  def open_tickets(self, title, **event_args):
    # This receieves a 'title' from the 'HeadlineStats' Form, 
    # which is one of "Unassigned", "Unresolved" and "Urgent"
    if title == "Unassigned":
      # If the "Unassigned" card is clicked, set self.filters['owner'] to the NO_AGENTS_SELECTED variable
      self.filters['owner'] =  Data.NO_AGENTS_SELECTED
    elif title == "Urgent":
      # If the "Urgent" card is clicked, set self.filters['owner'] to the NO_AGENTS_SELECTED variable
      self.filters['priority'] = Data.URGENT
    else:
      self.filters['status'] = Data.OPEN 
    homepage = get_open_form()
    homepage.open_tickets(initial_filters=self.filters, initial_date_filters=self.date_filters)
